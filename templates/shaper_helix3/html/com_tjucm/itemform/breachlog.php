				...
				<div class="row">
					<div class="row">
						<div class="col-sm-9  text-end">
							...
							// Compute bottom Need Help button data (separate from the inline one)
							$allowTicketForStaff  = ($this->client == 'com_tjucm.breachlog') ? true : false;
							$showNeedHelpBottom   = false;
							$ticketConditionDatas = '';
							if (isset($this->ticketConditionData->linkField) && ($this->ticketConditionData->isCreateTicket == 'true') && ($user->authorise('core.manageall', 'com_cluster') || $orgAdmin || $allowTicketForStaff))
							{
								$fieldLink = $this->form_extra->getField($this->ticketConditionData->linkField);
								if ($fieldLink && $fieldLink->value == '')
								{
									$showNeedHelpBottom   = true;
									$ticketConditionDatas = json_encode($this->ticketConditionData);
									$ticketConditionDatas = str_replace('"', '&quot;', $ticketConditionDatas);
								}
							}?>
							<?php if ($this->client == 'com_tjucm.breachlog') {
								if ($showNeedHelpBottom) { ?>
									<input type="button" class="btn btn-primary px-25 mobile-space" name="addTicketBottom" id="addTicketBottom" value="<?php echo Text::_('COM_DPE_ADDTICKET')?>" onclick="logticket.saveAndAddTicketBottom('<?php echo $ticketConditionDatas ?>', function(){ <?php if ($this->fieldTableLink->id){?> updateLinkField();<?php }?> });">
								<?php } 
							}?>
