...
class DpeControllerTjucm extends AdminController
{
    ...
    /**
     * Converts flexbox-based layouts inside Free Text HTML into mPDF-compatible tables.
     *
     * @param string $html The HTML string containing potentially flex-styled wrappers.
     * @return string The converted HTML.
     */
    private function convertFlexToTable($html)
    {
        // Regex to match a div with display:flex, containing an icon/badge div and a text div
        $pattern = '/<div\s+style="([^"]*display:\s*flex;?[^"]*)"[^>]*>\s*<div\s+style="([^"]*border-radius:\s*(50%|[0-9]+px);?[^"]*)"[^>]*>(.*?)<\/div>\s*<div[^>]*>(.*?)<\/div>\s*<\/div>/is';

        $html = preg_replace_callback($pattern, function($matches) {
            $outer_style_orig = $matches[1];
            $inner_style_orig = $matches[2];
            $icon_content = trim($matches[4]);
            $text_content = trim($matches[5]);

            // Clean outer styles (remove flex properties, keep background, borders, padding)
            $outer_style = preg_replace('/display:\s*flex;?|gap:\s*[^;]+;?|align-items:\s*[^;]+;?/i', '', $outer_style_orig);
            $outer_style = trim($outer_style, ' ;') . ';';

            // Extract width/height/background from inner badge styles dynamically
            $width = '36px';
            $height = '36px';
            $bg = '#d93838';
            $color = '#fff';
            ...
            if (preg_match('/width:\s*([^;]+)/i', $inner_style_orig, $w_match)) {
                $width = trim($w_match[1]);
            }
            if (preg_match('/height:\s*([^;]+)/i', $inner_style_orig, $h_match)) {
                $height = trim($h_match[1]);
            }
            if (preg_match('/background(-color)?:\s*([^;]+)/i', $inner_style_orig, $bg_match)) {
                $bg = trim($bg_match[2]);
            }
            if (preg_match('/color:\s*([^;]+)/i', $inner_style_orig, $c_match)) {
                $color = trim($c_match[1]);
            }
            if (preg_match('/font-size:\s*([^;]+)/i', $inner_style_orig, $fs_match)) {
                $font_size = trim($fs_match[1]);
            }
            if (preg_match('/font-weight:\s*([^;]+)/i', $inner_style_orig, $fw_match)) {
                $font_weight = trim($fw_match[1]);
            }

            // Extract numeric values for SVG dimensions safely
            $w_num = (int)preg_replace('/[^0-9]/', '', $width);
            $h_num = (int)preg_replace('/[^0-9]/', '', $height);
            if ($w_num <= 0) $w_num = 36;
            if ($h_num <= 0) $h_num = 36;
            $radius = $w_num / 2;
            $text_y = $radius + ($h_num * 0.2); // vertical centering calculation

            // Build the outer div + inner table HTML with inline SVG circle
            $new_html = '<div style="' . htmlspecialchars($outer_style) . '">';
            $new_html .= '<table style="width: 100%; border-collapse: collapse; border: none; background: transparent; margin: 0; padding: 0;" cellpadding="0" cellspacing="0">';
            $new_html .= '<tr>';
            $new_html .= '<td style="width: ' . htmlspecialchars($width) . '; vertical-align: middle; padding-right: 14px; border: none; background: transparent;">';

            $new_html .= '<svg width="' . $w_num . '" height="' . $h_num . '" viewBox="0 0 ' . $w_num . ' ' . $h_num . '">';
            $new_html .= '<circle cx="' . $radius . '" cy="' . $radius . '" r="' . $radius . '" fill="' . htmlspecialchars($bg) . '" />';
            $new_html .= '<text x="' . $radius . '" y="' . $text_y . '" font-family="Arial, Helvetica, sans-serif" font-size="' . htmlspecialchars($font_size) . '" font-weight="' . htmlspecialchars($font_weight) . '" fill="' . htmlspecialchars($color) . '" text-anchor="middle">' . htmlspecialchars($icon_content) . '</text>';
            $new_html .= '</svg>';

            $new_html .= '</td>';
            $new_html .= '<td style="vertical-align: middle; border: none; background: transparent; padding: 0;">';
            $new_html .= $text_content;
            $new_html .= '</td>';
            $new_html .= '</tr>';
            $new_html .= '</table>';
            $new_html .= '</div>';
    ...
    {
        $isFreetext = str_contains($field->label, "freetextMod");

        // Check if 'freetextMod' exists in the label
        if ($isFreetext) {
            // Convert any flex layouts inside the free text HTML to mPDF-compatible tables
            $field->label = $this->convertFlexToTable($field->label);

            // Replace only the first div class (the outer wrapper) with inline width style
