...
class PlgSystemTjucmdpe extends CMSPlugin
{
	...
	public function onUcmItemAfterSave($validData, $extra_jform_data, $isNew)
	{
		...
		if (count((array) $isChecklist) > 0) {
			...
			$dpeAdminUsers = array();
			// Code comment to handle the dpeadmins from global insted of the license .
			...
			// } else {
			$dpeAdminUsers = $allDpeSelectedAdmins; // Adding global config admins into the dpeadminusers variable for the next step process
			// }
			// }
			...
			if (!$isNew) {
				...
				$replacements->checklist->checklistUrl = Uri::root() . substr(
					Route::_('index.php?option=com_dpe&view=dashboard'),
			...
			if ($isNew) {
				...
				if ($client == 'com_tjucm.dpialite') {
					...
					$dpeAdminUsersDpia = $allDpeSelectedAdmins; // Add all dpe admin users from global config.
