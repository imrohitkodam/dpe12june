...
calculateNumeriFieldData = (configFieldNames, configColorCombination, processdFieldName, fieldId, defaultText, showCalculation = 0) => {
    ...
    setTimeout(function () {
            ...
            if (value) {
                ...
                if (listTypeValue == 'select') {
                    if (jQuery('#jform_' + value).parent().parent().attr('style') != 'display: none;') {
                        var selectedVal = jQuery('#jform_' + value).val();
                        calculation[index] = (selectedVal) ? selectedVal : 0;
                    }
                    else {
                        nonValue[index] = 0;
                    }
                    ...
                    for (i = 0; i < radioIDs; i++) {
                        ...
                        if ((radioElement.is(':checked')) && ((notHidden !== 'display: none;'))) {
                            selectedVals.push(radioElement.val());
                        }
                    }

                    if (selectedVals.length > 0) {
                        calculation[index] = selectedVals.length > 1 ? selectedVals : selectedVals[0];
                    } else {
                        nonValue[index] = 0;
                    }
        ...
        jQuery.each(calculation, function (index, value) {
            let valToUse = value;
            let regex = new RegExp("\\b" + fieldName[index] + "(?:\\|([+\\-*/]))?(?!\\w)", "g");

            jsonFieldName = jsonFieldName.replace(regex, function(match, operator) {
                if (Array.isArray(valToUse)) {
                    let validNums = valToUse.map(v => parseFloat(v)).filter(v => !isNaN(v));
                    if (validNums.length === 0) validNums = [0];
                    let op = operator ? operator : '+';
                    return '(' + validNums.join(' ' + op + ' ') + ')';
                } else {
                    let singleVal = valToUse;
                    if (singleVal == undefined || isNaN(singleVal) || singleVal === '') {
                        singleVal = 0;
                    }
                    return singleVal;
                }
        ...
        jsonFieldName = jsonFieldName.replace(/\{/g, '');
        jsonFieldName = jsonFieldName.replace(/\}/g, '');
        ...
        if (showCalculation) {
            // jQuery('#' + targetId + '_span').attr('data-calculation', jsonFieldName + ' = ' + total);
        jQuery('#' + targetId + '_span').attr('title', jsonFieldName + ' = ' + total);

        } else {
            // jQuery('#' + targetId + '_span').removeAttr('data-calculation');
        }
...
calculateNumeriFieldDataSubform = (configColorcombinations, processdFieldName, configFieldNames, currentFieldName, defaultText, showCalculation = 0) => {
    ...
    for (let indexCount = 0; indexCount < subFormCount; indexCount++) {
        ...
        if (showCalculation) {
            jQuery('#jform_' + subformFieldName + '__' + subformFieldName + indexCount + '__' + calculationFieldName + '_span').attr('data-calculation', jsonFieldName + ' = ' + total);
            jQuery('#' + subformFieldName + indexCount + '_span').attr('data-calculation', jsonFieldName + ' = ' + total);
        } else {
            jQuery('#jform_' + subformFieldName + '__' + subformFieldName + indexCount + '__' + calculationFieldName + '_span').removeAttr('data-calculation');
            jQuery('#' + subformFieldName + indexCount + '_span').removeAttr('data-calculation');
        }
...
calculationOnchangeSubform = (event, colorcombinations, processdFieldName, configFieldNames, currentFieldName, defaultText, showCalculation = 0) => {
    ...
    if (calculation.length > 0 || nonValue.length > 0) {
        ...
        if (showCalculation) {
            if (listValue == 'select') {
                jQuery('.subform-repeatable-group[data-group="' + newId + '"]').find('span[id$=' + calculationFieldId + '_span]').attr('data-calculation', jsonFieldName + ' = ' + total);
                jQuery('.subform-repeatable-group[data-group="' + newId + '"]').find('span[id$=' + existingCalculationFieldId + '_span]').attr('data-calculation', jsonFieldName + ' = ' + total);
            } else {
                ...
                numericCalculationSpan.attr('data-calculation', jsonFieldName + ' = ' + total);
            }
        } else {
            if (listValue == 'select') {
                jQuery('.subform-repeatable-group[data-group="' + newId + '"]').find('span[id$=' + calculationFieldId + '_span]').removeAttr('data-calculation');
                jQuery('.subform-repeatable-group[data-group="' + newId + '"]').find('span[id$=' + existingCalculationFieldId + '_span]').removeAttr('data-calculation');
            } else {
                var containerDiv = jQuery(event.target).closest('.subform-repeatable-group');
                var numericCalculationSpan = containerDiv.find('.numericcalculation');
                numericCalculationSpan.removeAttr('data-calculation');
            }
        }
