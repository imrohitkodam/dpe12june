...
INSERT INTO `#__autotweet_channeltypes`
	(`id`, `name`, `description`, `max_chars`,`joocial_exclusive`,`frontend`)
	VALUES (19, '― Deprecated ― EasySocial', 'COM_AUTOTWEET_CHANNEL_EASYSOCIAL_DESC', 420, 1, 0);
...
ALTER TABLE `#__autotweet_channels` DROP `scope`;

ALTER TABLE `#__autotweet_channels` CHANGE `media_mode` `media_mode`
  VARCHAR(24) NOT NULL DEFAULT 'message-with-image';

UPDATE `#__autotweet_channels` SET `media_mode` = 'message-with-image' WHERE `media_mode` = 'attachment';

UPDATE `#__autotweet_channels` SET `media_mode` = 'message-with-image' WHERE `media_mode` = 'both';
